...
var dataMap = {
    'home_url': 'hot.json',
    'hot_url': 'hot.json',
    'douyu_url': 'douyu.json',
    'huya_url': 'huya.json',
    'longzhu_url': 'longzhu.json',
    'zhanqi_url': 'zhanqi.json',
    'xiongmao_url': 'xiongmao.json',
    'star_url': 's.json',
    'qiu_url': 'q.json',
    'lian_url': 'b.json',
    'meng_url': 'm.json',
...
function addHref(){
    var arr = document.querySelectorAll('.addHref');
    var l = arr.length;
    for(var i = 0; i < l; i++){
        var elemID = arr[i].id;
        var href = DOMAIN + url[elemID];
        arr[i].setAttribute('href', href);
        arr[i].setAttribute('data-aim', dataMap[elemID]);
    }
}
/**
...
$(function(){
    function handleImgs(){
        var liveElems = $('#mainContent .pos-rlt img');
        var showNums = (liveElems.length >= 12)? 12 : liveElems.length ;
        for(var i = 0; i < showNums; i++){
            var _liveElem = liveElems.eq(i);
            _liveElem.attr('src',_liveElem.attr('data-original'));
            _liveElem.removeClass('lazy');
            if(i == (showNums - 1)){
                liveElems.eq(0).load(function(){
                    var imgHeight = liveElems.eq(0).height();
                    console.log('height:' + imgHeight);
                    $('#mainContent .pos-rlt img').height(imgHeight);
                    $('img.lazy').lazyload({
                        container: $("#mainContent"),
                        placeholder: "http://7xlknn.com1.z0.glb.clouddn.com/default.png"
                    });
                })
            }
        }
...
+function ($) { "use strict";
    ...
    Bjax.prototype.start = function () {
        ...
        $.ajax(this.options.url + '/index.html').done(function(r){
            that.$content = r;
            that.complete();
        });
    }
    ...
    function fillHtml(aim,elem){
        var jsonUrl = 'http://7xlknn.com1.z0.glb.clouddn.com/' + aim + '?now=' + new Date().getTime();
        console.log(elem);
        $.get(TEMPLATE_URL, function(template) {
            console.log('获得模板')
            $.getJSON(jsonUrl,function(pageData){
    ...
    Bjax.prototype.update = function (){
        ...
        if( !this.$element.is('html') ){
            ...
            var elem = this.$element.find('#mainContent');
            var aim = this.aim;

            fillHtml(aim,elem);
            //this.$element.find('#mainContent').html('');
